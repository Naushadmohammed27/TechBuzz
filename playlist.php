   ...
   <div class="container row">
      ...
      <div class="col">
         ...
         <div class="details">
            ...
            <!-- 🔥 STEP 5 ADDED CODE: Certificate Button -->
            <?php
            if($user_id != ''){
               $check_progress = $conn->prepare("SELECT * FROM progress WHERE user_id = ? AND playlist_id = ? LIMIT 1");
               $check_progress->execute([$user_id, $playlist_id]);
               ...
               if($check_progress->rowCount() > 0){
                  ...
                  if($progress['is_completed'] == 1){
                     echo '
                     <a href="certificate/generate_certificate.php?user_id='.$user_id.'&playlist_id='.$playlist_id.'" 
                        class="btn" 
                        style="background:#1abc9c; color:white; padding:10px 15px; border-radius:5px; margin-top:15px; display:inline-block;">
