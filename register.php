...
if (isset($_POST['submit'])) {
   ...
   if ($select_user->rowCount() > 0) {
      ...
      // ✅ Store raw password directly (Not secure for production)
      $plain_pass = $pass;

      $insert_user = $conn->prepare("INSERT INTO `users`(id, name, email, password, image) VALUES(?,?,?,?,?)");
      $insert_user->execute([$id, $name, $email, $plain_pass, $rename]);
      ...
      if ($verify_user->rowCount() > 0) {
         ...
         if ($pass === $row['password']) {  // ✅ Direct comparison
...
<head>
   ...
   <title>Register Now - Welcome to TechBuzz</title>
